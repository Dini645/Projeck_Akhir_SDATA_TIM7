...
bool isValidNIK(const char* nik) {
    // Validasi NIK: minimal 4 karakter, maksimal 20 karakter
    // Bisa berisi huruf atau angka
    int len = stringLength(nik);
    if (len < 4 || len > 20) return false;
    
    // Validasi karakter: hanya boleh huruf (A-Z, a-z) atau angka (0-9)
    for (int i = 0; i < len; i++) {
        if (!((nik[i] >= 'A' && nik[i] <= 'Z') ||
              (nik[i] >= 'a' && nik[i] <= 'z') ||
              (nik[i] >= '0' && nik[i] <= '9'))) {
...
// CustomerBinaryTree Implementation
...
void CustomerBinaryTree::insertRecursive(TreeNode* &node, const char* name, const char* nik, int amount) {
    ...
    // Jika NIK sama atau salah satu lebih pendek
    if (nik[i] == '\0' && node->nik[i] == '\0') {
        // NIK sama, update data
...
// CustomerAVLTree Implementation
...
TreeNode* CustomerAVLTree::insertRecursive(TreeNode* node, const char* name, const char* nik, int amount) {
    ...
    if (compare == 0) {
        // NIK sama atau salah satu lebih pendek
        if (nik[i] == '\0' && node->nik[i] == '\0') {
            // NIK identik, update data
            copyString(node->name, name, 50);
            node->amount += amount;
            return node;
        } else if (nik[i] == '\0') {
...
// TransactionHistory Implementation
...
// DriveThruSystem Implementation
